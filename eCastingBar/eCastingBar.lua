-- Global constants
CASTING_BAR_MAJOR_VERSION = "4";
CASTING_BAR_MINOR_VERSION = "0";
CASTING_BAR_REVISION = "1";
...
function eCastingBar_OnEvent(self, newevent, ...)
	...
	elseif( newevent == "UNIT_SPELLCAST_CHANNEL_START" ) then
		local spellName, spellRank, displayName, texture, startTime, endTime, isTradeSkill, notInterruptible = UnitChannelInfo(newarg1)
		if ( not spellName) then
			self:Hide();
			return;
		end
	...
	elseif ( newevent == "UNIT_SPELLCAST_INTERRUPTIBLE" ) then
		if eCastingBar_Saved[frame.."HideBorder"] == 0 then
			self:SetBackdrop({bgFile = CASTING_BAR_BACKGROUND_FILE, edgeFile = CASTING_BAR_EDGE_FILE_UNINT, tile = true, tileSize = 16, edgeSize = 16, insets = { left = 2, right = 2, top = 2, bottom = 2 }}) 
			self:SetBackdropColor(0,0,0,0.5)
		end
	elseif ( newevent == "UNIT_SPELLCAST_NOT_INTERRUPTIBLE" ) then
		if eCastingBar_Saved[frame.."HideBorder"] == 0 then
			self:SetBackdrop({bgFile = CASTING_BAR_BACKGROUND_FILE, edgeFile = CASTING_BAR_EDGE_FILE_UNINT, tile = true, tileSize = 16, edgeSize = 16, insets = { left = 2, right = 2, top = 2, bottom = 2 }}) 
			self:SetBackdropColor(0,0,0,0.5)
		end
